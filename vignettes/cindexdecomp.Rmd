...
form: every comparable pair is weighted by $1/\hat{G}(T_i)^2$.
...
agree exactly when event times are distinct, and diverge slightly once
times are tied:

```{r}
tie_agreement <- function(time, status, risk) {
  ref_h <- concordance(Surv(time, status) ~ risk, reverse = TRUE)$concordance
  ref_u <- concordance(Surv(time, status) ~ risk, reverse = TRUE,
...
comparable pairs. Model-based estimators such as Gonen-Heller integrate
