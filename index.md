...
Because [`confint()`](https://rdrr.io/r/stats/confint.html) bootstraps
the gap itself — resampling subjects, not pairs — you can say whether
that gap is distinguishable from zero.
...
The pooled result agrees with
[`survival::concordance()`](https://rdrr.io/pkg/survival/man/concordance.html)
to machine precision for Harrell’s weighting under every tie pattern,
and for Uno’s weighting when event times are distinct. Under tied times,
...
![Global C-index and event-event C-index plotted against the cohort
censoring rate. The global curve stays roughly flat near 0.65 while the
event-event curve falls towards and below 0.5, and the shaded band
between them is the masking gap.](reference/figures/README-curve-1.png)
...
re-exported, so **ggplot2 does not need to be attached** to get the plot
— only to modify it afterwards.
...
![Dumbbell plot with one row per model, each row spanning that model's
event-event and event-censored C-index with bootstrap error
bars.](reference/figures/README-compare-1.png)
...
- `C_ee` is imprecise when few event-event pairs exist. Use
  [`confint()`](https://rdrr.io/r/stats/confint.html); pair counts
  describe composition, not precision.
